...
Boolean testavendas(Lista lista,FILE *file,Cat_Produtos cat_produtos,Cat_Clientes cat_clientes){
  ...
  printf("Vendas     |                |               |\nValidas    |      %7d   | %10f |  %s\n           |                |               |\n",vendas_validas,aux->quantidade,(vendas_validas==aux->quantidade ? "SIM" : "NÂO"));
  printf("-----------------------------------------------------\n");
  for(aux=lista;aux!=NULL;aux=aux->next)
    if(aux->tipo==Vendas_invalidas) break;
  printf("Vendas     |                |               |\nInvalidas  |      %7d   | %10f |  %s\n           |                |               |\n",vendas_invalidas,aux->quantidade,(vendas_invalidas==aux->quantidade ? "SIM" : "NÂO"));
  printf("-----------------------------------------------------\n");
...
int main(){
    ...
    FILE *file_teste_produtos=NULL;
        ...
        testadados(produtos,clientes,faturacao,filiais,testes);
        ...
        */
        ...
        testaqueries(produtos,faturacao,filiais);
