#include "./headers/interpretador.h"

#define MENU_PRINCIPAL 1
#define SAIR_PROGRAMA 0


void menu_principal(Cat_Produtos produtos, Cat_Clientes clientes, Faturacao faturas) {

	int estado = MENU_PRINCIPAL, input = 0;
	char opcao[20];
		
	while(estado) {
		printf("\e[1;1H\e[2J");
		printf( "_____________________________________________\n" );
		printf( "\n\t\tMENU PRINCIPAL\n\n" );
		printf( "---------------------------------------------\n" );
		printf(" Escolha uma das seguintes opções:\n\n" );
		printf("  1. Leitura de Ficheiros\n");
		printf("  2. Catálogo de Produtos\n");
		printf("  3. Faturação\n");
		printf("  4. Filiais\n");
		printf( "---------------------------------------------\n" );
		printf( "\t\t\t\t0 - Sair\n" );
		printf( "_____________________________________________\n" );
		
		printf("\nEscolha uma opção >> ");
		
		int input = scanf("%s",opcao);
		
		switch(opcao[0]) {
			
			case '0': estado = SAIR_PROGRAMA; break;
			
			case '1': estado = menu_leitura(produtos,clientes,faturas); break;

			case '2': estado = menu_catalogo(produtos); break;
			
			case '3': estado = menu_faturacao(faturas); break;
		/*	
			case '4': estado = menu_filiais(); break; */
			
			default: break;
		}
	}
}


int menu_leitura(Cat_Produtos produtos, Cat_Clientes clientes,Faturacao faturas) {

	int estado = 1; 
	int input = 0;
	char opcao[20];

	while(estado) {
	
		printf("\e[1;1H\e[2J");
		printf("_____________________________________________\n");
		printf("\n\t\tMENU LEITURA\n\n");
		printf("---------------------------------------------\n");
		printf(" Escolha uma das seguintes opções:\n\n");
		printf("  1. Carregar ficheiros pré-definidos\n\n");
		printf("  2. Introduzir ficheiros\n\n");
		printf("---------------------------------------------\n");
		printf("  3 - Voltar\t\t\t0 - Sair\n" );
		printf("_____________________________________________\n");
		
		printf("\nEscolha uma opção >> ");
		
		int input = scanf("%s",opcao);
		
		switch(opcao[0]) {
			
			case '0': return SAIR_PROGRAMA; break;
			
			case '1': estado = querie_1(produtos,clientes,faturas,1); break;
			
			case '2': estado = querie_1(produtos,clientes,faturas,2);  break;

			case '3': return MENU_PRINCIPAL; break;

			default: break;
		}

	}
	return estado;
}


int menu_catalogo(Cat_Produtos produtos) {

	int estado = 1; 
	int input = 0;
	char opcao[20];

	while(estado) {
	
		printf("\e[1;1H\e[2J");
		printf("_____________________________________________\n");
		printf("\n\t\tMENU CATALOGOS\n\n");
		printf("---------------------------------------------\n");
		printf(" Escolha uma das seguintes opções:\n\n");
		printf("  1. Listagem de Produtos por letra\n\n");
		printf("---------------------------------------------\n");
		printf("  2 - Voltar\t\t\t0 - Sair\n" );
		printf("_____________________________________________\n");
		
		printf("\nEscolha uma opção >> ");
		
		int input = scanf("%s",opcao);
		
		switch(opcao[0]) {
			
			case '0': return SAIR_PROGRAMA; break;
			
			case '1': estado = querie_2(produtos); break;
			
			case '2': return MENU_PRINCIPAL; break;

			default: break;
		}

	}
	return estado;
}



int menu_faturacao(Faturacao faturas) {

	int estado = 1;
	int input = 0;
	char opcao[20];

	while(estado) {
	
		printf("\e[1;1H\e[2J");
		printf("_____________________________________________\n");
		printf("\n\t\tMENU FATURACAO\n\n");
		printf("---------------------------------------------\n");
		printf(" Escolha uma das seguintes opções:\n\n");
		printf("  1. Total de vendas/faturação dado um mês\n");
		printf("  2. Produtos não comprados\n");
		printf("  3. Vendas/faturação num intervalo de meses\n\n");
		printf("---------------------------------------------\n");
		printf("  4 - Voltar\t\t\t0 - Sair\n");
		printf("_____________________________________________\n");
		
		printf("\nEscolha uma opção >> ");
		
		input = scanf("%s",opcao);
		
		switch(opcao[0]) {
			
			case '0': return SAIR_PROGRAMA; break;
			
			case '1': estado = querie_3(faturas); break;
			
			case '2': estado = querie_4(faturas); break;

			case '3': estado = querie_6(faturas); break;
			
			case '4': return MENU_PRINCIPAL; break;

			default: break;
		}
	}
	return estado;
}